...
def gen_sim_matrix(path_input, path_output, sim_opt_id):
    ...
    if sim_type == 'feat-based': gen_feat_based_matrix(path_input, path_output, sim_opt_id)
    elif sim_type == 'image-based': print(f"{sim_opts['description']} not supported yet...") # gen_image_based_matrix(path_input, path_output_ sim_opt_id)
